...
const data = fs.readFileSync('../2019 Solutions/inputs/day13input.txt').toString();
...
// run program to get initial outputs
let initialRun = runProgram();
let max = Math.max(...initialRun);

// for every 3 numbers:
// 1st is x position (in from left)
// 2nd is y position (down from top)
// 3rd is type of tile - 
// 0 is empty
// 1 is wall (indestructible)
// 2 is block (can be broken with ball)
// 3 is paddle (indestructible)
// 4 is ball (moves diagonally)
let board = [];
let numBlocks = 0;

for (let h = 0; h < max; h++) {
  let row = Array.from(Array(max));
  board.push(row);
}


for (let i = 0; i < initialRun.length; i += 3) {
  let x = initialRun[i];
  let y = initialRun[i+1];
  let type = initialRun[i+2];
